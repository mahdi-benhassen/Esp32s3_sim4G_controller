...
The next tranche should add Ethernet behind a verified PHY profile, transport-priority failover, authenticated rule CRUD, and retained discovery for storage/RTC/modem state. The current services already share versioned MQTT/HTTP state shapes, and the optional PPP service is present, but hardware-in-the-loop cellular/carrier testing is still required before field deployment.
